...
namespace SchoolV01.Client.Infrastructure.Managers.Identity.Roles
{
    public class RoleManager : IRoleManager
    {
        ...
        public async Task<IResult<string>> UpdatepostPermissionsAsync(PermissionRequest request)
        {
            var response = await _httpClient.PostAsJsonAsync(Routes.RolesEndpoints.UpdatepostPermissions, request);
            return await response.ToResult<string>();
        }
    }
}