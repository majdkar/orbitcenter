...
namespace SchoolV01.Client.Infrastructure.Managers.Identity.Users
{
    public class UserManager : IUserManager
    {
        ...
        {
            var response = await _httpClient.GetAsync(Routes.UserEndpoints.GetByEmail(userEmail));
            return await response.ToResult<UserResponse>();
        }
        ...
        public async Task<IResult> ResetPasswordAsync(ResetPasswordUserRequest request)
        {
            var response = await _httpClient.PostAsJsonAsync(Routes.UserEndpoints.ResetPasswordUser, request);
            return await response.ToResult();
        }
