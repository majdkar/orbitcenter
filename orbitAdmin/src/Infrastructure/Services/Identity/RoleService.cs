...
namespace SchoolV01.Infrastructure.Services.Identity
{
    public class RoleService : IRoleService
    {
        ...
        public async Task<Result<List<RoleResponse>>> GetAllAsync()
        {
            ...
            var rolesResponse = _mapper.Map<List<RoleResponse>>(roles.Where(x => x.Name != RoleConstants.AdministratorRole));
            ...
            if (role != null)
            {
                ...
                if (roleClaimsResult.Succeeded)
                {
                    ...
                    foreach (var permission in allPermissions)
                    {
                        ...
                        //  permission.Value= permission.Value.Split('.').LastOrDefault();
                    }
                }
                else
                {
        ...
        public async Task<Result<string>> SaveAsync(RoleRequest request, bool isAdmin = false)
        {
            ...
            else
            {
                ...
                if (existingRole.Name == RoleConstants.AdministratorRole || existingRole.Name == RoleConstants.BasicRole || !isAdmin)
                {
        ...
        public async Task<Result<string>> UpdatePermissionsAsync(PermissionRequest request)
        {
            try
            {
                ...
                {
                    addedClaims.Messages.Add("This Message is By Ahmad");
