...
namespace SchoolV01.Server.Controllers
{
    ...
    public class RoleController : ControllerBase
    {
        ...
        {
            var response = await _roleService.SaveAsync(request, User.IsInRole(RoleConstants.AdministratorRole));
            return Ok(response);
        }
        ...
        /// <returns></returns>
        [Authorize(Policy = Permissions.RoleClaims.Edit)]
        [HttpPost("permissions/updateall")]
        public async Task<IActionResult> Updateall(PermissionRequest model)
        {
            var response = await _roleService.UpdatePermissionsAsync(model);
            return Ok(response);
        }
